...
* 完整音频已经生成时，页面按真实分句 WAV 时长和句后停顿显示每个场景的音频开始时间、结束时间和时长。尚无音频时，场景卡片省略音频时间。
* 关键帧提供八种插画分镜风格和电影写实风格。每个场景可以独立选择风格并重新生成单张，也可以选择统一风格重新生成全量。
...
IndexTTS 与 VoiceDesign 模型分别使用 Docker 命名卷。后台生成期间，页面显示实际运行时进程状态、模型阶段、进程内存、累计磁盘读取量和模型总字节数，便于区分首次加载与驻留模型复用。
