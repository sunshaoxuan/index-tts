...
* 分镜工作区可以为本批单张或全量关键帧选择主图像模型或互补图像模型，并独立决定是否允许冷却切换。只有 429、配额冷却、资源耗尽或 503 临时不可用会触发互补模型；参考图格式、Images Edits 能力和其他请求错误保持失败。发生切换时，每张镜头保存请求模型、实际模型、切换原因和模型专用提示词规格，镜头卡显示最近实际模型。
