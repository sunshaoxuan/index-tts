...
* 每个角色按当前配置生成一至六个候选，按年龄与性别门禁和实测基频选择最佳样本，并在永久音色元数据中保存逐候选测量记录。
...
每个角色可以填写一段专属试听文本，并生成一至六个候选。候选在同一个驻留模型中连续生成，全部保存到永久音色库。角色卡片会显示每个候选的种子、实测中位基频和试听控件，点击“采用此候选”只切换当前角色的稳定音色。
