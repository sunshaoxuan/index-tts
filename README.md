...
* 每个场景保存由 AI 撰写的场景小记，具体描述环境、人物位置与动作、前后景、镜头方向、光线、色彩和关键物件，可直接作为关键帧生成依据。
* 每个场景包含多个连续分镜镜头，一个镜头对应一张关键帧。默认按约 10 秒一个镜头拆分，也可选择 5、8、12、15 或 20 秒；长音频会形成与时长相匹配的镜头数量。
...
* 支持手工创建镜头、按中间分句拆分镜头，以及勾选同一场景内相邻镜头进行合并。镜头范围变化后会清除不再匹配的旧关键帧，并提示保存工程。
* 关键帧提供八种插画分镜风格和电影写实风格。每个镜头可以独立选择风格并重新生成单张，也可以选择统一风格重新生成全部镜头图片。
* 关键帧生成会把当前场景小记、主题、地点、空间方位、故事内时间、叙事视角、基调、参与人物小传和对应角色形象发送到当前配置的兼容图像服务。画面角色按稳定角色 ID 匹配，旁白不作为画面人物。存在画面角色时使用 Images Edits 多图参考，每次单张或全量生成都复用角色资产中的原始角色图，约束面部结构、五官比例、发型、年龄感和标志特征，避免以上一张关键帧递推造成累积漂移。缺少角色形象或服务不支持多图编辑时会停止生成并显示原因。生成图片、提示、风格、模型、时间、参考模式、角色 ID、角色图 URL 和 SHA256 保存在工程中，图片文件位于工程专属 `scene-assets` 目录。
