...
IndexVoiceStudio 是面向小说、故事和新闻长文本的本地声音与视频分镜制作工作台。它在 IndexTTS 2.5 推理能力之上提供 AI 全文导演、视频分镜、关键帧生成、角色音色设计、可编辑分句、逐句增量重生成、纠音规则、完整音频拼接和分轨交付。
...
* 可配置 OpenAI 兼容服务扩写详细人物小传，并通过 GPT Image、Gemini Image 或兼容图像模型生成稳定人物形象。图像设置支持主模型、互补模型和冷却切换。
...
* AI 按内容主题建立视频分镜。小说中的地点、室内外、人物方位、观察方向或叙事焦点变化也会建立新场景，并保存可复核的场景切换依据、原文证据和起止分句。
* 每个场景保存由 AI 撰写的场景小记，作为地点、方位和视觉连续性的场景背景。每个镜头另有独立画面小记，AI 必须读取该镜头起止分句和完整对应原文，再描述当前镜头的可见主体、动作、前后景、构图、景别、光线、色彩和关键物件，并按稳定角色 ID 标记实际出现在画面中的人物。小说旁白只是声音线索；系统还会把原文明示姓名和场景内唯一代词连续指向的人物确定性并入镜头人物列表，避免角色图在连续镜头中丢失。
* 每个场景包含多个连续分镜镜头，一个镜头对应一张关键帧。默认按约 10 秒一个镜头拆分，也可选择 5、8、12、15 或 20 秒；长音频会形成与时长相匹配的镜头数量。镜头卡直接显示完整对应原文和 AI 原文取景证据，便于核对画面小记是否真正随音频内容推进。
* 完整音频已经生成时，镜头拆分使用真实分句 WAV 时长和句后停顿，并显示每个镜头的音频开始时间、结束时间和时长。尚无音频时按文字长度估算边界并省略音频时间。
* 分镜页可以直接 AI 重新生成全部场景与镜头，同时保留角色、当前分句、人工朗读文字、音频、纠音和导演记忆。
* 支持手工创建镜头、按中间分句拆分镜头，以及勾选同一场景内相邻镜头进行合并。镜头范围变化后会重新计算对应原文，并清除不再匹配的旧画面小记、取景证据和关键帧，要求使用者根据新范围补写或执行 AI 全量分镜后再生图。
* 关键帧提供八种插画分镜风格和电影写实风格。每个镜头可以独立选择风格并重新生成单张，也可以选择统一风格重新生成全部镜头图片。
* 关键帧生成会把当前镜头对应原文、独立画面小记、主题、地点、空间方位、故事内时间、叙事视角、基调、参与人物小传和对应角色形象发送到当前配置的兼容图像服务。画面角色按稳定角色 ID 匹配，旁白不作为画面人物。存在画面角色时使用 Images Edits 多图参考，每次单张或全量生成都复用角色资产中的原始角色图，约束面部结构、五官比例、发型、年龄感和标志特征，避免以上一张关键帧递推造成累积漂移。缺少角色形象或服务不支持多图编辑时会停止生成并显示原因。生成图片、提示、风格、模型、时间、参考模式、角色 ID、角色图 URL 和 SHA256 保存在工程中，图片文件位于工程专属 `scene-assets` 目录。
* 全量关键帧在第一张图片请求前一次性预检全部镜头，随后按分镜顺序逐张生成并立即回写。进度面板持续显示完成数、总数、百分比、当前镜头、已用时间和预计剩余时间。生成期间锁定分镜编辑、镜头合并与拆分、工程切换、保存、AI 设置和其他冲突生成入口；使用者可以取消当前单张或全量任务。完成、取消或失败后自动解除锁定，已经成功的图片继续保留并可保存，失败状态会标明具体镜头。
* 分镜工作区可以为本批单张或全量关键帧选择主图像模型或互补图像模型，并独立决定是否允许冷却切换。429、500、502、503、504、请求超时、连接中断、配额冷却、资源耗尽或临时不可用会触发互补模型；参考图格式、Images Edits 能力和普通 4xx 请求错误保持失败。发生切换时，每张镜头保存请求模型、实际模型、切换原因和模型专用提示词规格，镜头卡显示最近实际模型。上游 5xx 最终失败时，Product Studio 使用 424 JSON 返回具体原因，避免公网反向代理把错误体替换为 HTML。
* 所有长时间操作都提供就近取消入口。统一后台任务取消覆盖单分句与完整音频、角色音色、全文分析和 AI 分镜；请求级取消覆盖人物扩写、角色形象、单张与全量关键帧、AI 服务测试和工程读取。取消会停止对应 Worker、归属明确的音频 Runtime 或上游 HTTP 请求，解除界面锁定并保留此前已经成功生成的成果。
...
产品默认连接 `http://127.0.0.1:11434`，全文分析默认使用 `qwen3:14b`。
...
IndexTTS 与 VoiceDesign 模型统一使用只读外部 Docker 卷 `indextts25-models-ssd`。该卷指向 C 盘 NVMe SSD 的正式模型目录。后台生成期间，页面显示实际运行时进程状态、模型阶段、进程内存、累计磁盘读取量和模型总字节数，便于区分首次加载与驻留模型复用。
...
1. 新建工程，选择体裁。需要复用既有资产时，可多选关联来源工程，一次导入角色资料、当前音色、全部候选音色和全篇纠音规则。多来源纠音按选择顺序合并，同原词的完全重复规则只保留一条，内容冲突时保留首个来源规则，重复和冲突均写入关联导入回执。新文章完成 AI 分析后，系统按角色类型与标准化名称匹配已有角色；重叠角色复用原角色 ID、资产与音色，新角色追加到资产库，未在本篇出现的既有角色继续保留。导入完成后各工程独立维护。
2. 执行 AI 全文分析，复核角色、章节、视频分镜和分句。视频分镜页可以修改内容主题、地点、空间方位、场景小记和切换依据。
3. 按需选择关键帧风格、当前批次图像模型和冷却切换开关，再生成单张或全量关键帧。完整音频生成后，返回视频分镜页核对每个场景的音频开始与结束时间。
4. 打开人物卡片，核对性别和年龄，完善详细人物小传，调整结构化声音特征、目标频率、生成策略和角色专属试听文本，并按需生成角色形象。
5. 在分句导演中调整合成文本、角色、情绪、节奏和停顿。
5. 逐句生成和试听需要修改的片断。
...
工程控制区提供“全局 AI 设置”。该设置统一控制全文分析、人物小传和图像生成，所有工程共用。全文分析可以选择本地 Ollama 或 OpenAI 兼容 Endpoint，并分别设置全文模型、Ollama 地址和分析块长度。兼容服务继续配置 Endpoint、API Key、兼容文本接口、人物小传模型、主图像模型、互补图像模型和冷却切换。全文分析选择兼容 Endpoint 时与人物小传共用该文本接口。Endpoint 推荐填写包含 `/v1` 的基础地址，例如：

```text
http://ccnode.briconbric.com:49530/v1
```

人物小传模型可以配置为兼容服务实际暴露的 Gemini、Claude 或其他文本模型名。主图像模型和互补图像模型必须使用服务实际暴露且彼此不同的图像模型 ID，例如 `gpt-image-2` 与 `gemini-3-pro-image`。人物小传可以选择 `/responses` 或 `/chat/completions`。无人物参考的图像使用 `/images/generations`，含人物参考的分镜使用 `/images/edits`。

填写 Endpoint 与 API Key 后，点击“测试兼容 Endpoint 并加载模型”。系统调用兼容 `/models` 接口并把当前 Key 可用的模型加载到可搜索下拉框，图像选择器优先显示包含 `image` 的模型 ID。模型列表没有包含服务端支持的别名时，也可以直接输入模型名。当前选择不在可用列表或两个图像模型相同时，设置窗口会显示警告。
...
全文分析选择兼容 Endpoint 时，系统会发送当前工程原文。调用小传扩写时，系统会发送角色名称、年龄、性别、当前小传和稿件中该角色附近的证据。生成角色形象时发送角色名称、年龄、性别、人物小传和角色形象提示。生成包含人物的分镜关键帧时，还会发送镜头参与人物的已保存角色图片作为身份参考；这些图片会产生图像输入 token 和相应费用。API Key 保存于本机 `runtime-output/product-settings.json`，读取接口只返回是否已经配置。
...
每个角色可以填写一段专属试听文本，并请求一至六个合格候选。存在目标基频时，系统最多执行缺少数量十二倍且不超过三十六次的内部尝试，达到所需合格数量后停止。每个同时通过年龄、性别和目标频率门禁的自然候选都会立即进入永久音色库和角色卡片；本轮未补齐时保留已合格候选，保存累计尝试数，并在下一轮只补缺口且使用新的连续种子。任务结果分别显示声学年龄或性别失败数与目标频率失败数。卡片显示种子、实测中位基频、门禁结果和试听控件。旧候选缺少当前校验版本时标记为历史候选待重新校验。点击“采用此候选”只切换当前角色的稳定音色。
