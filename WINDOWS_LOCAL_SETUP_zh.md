...
4. 情绪强度和句后停顿使用受限数值单元格。
5. VoiceDesign 原生自然语言能力通过“高级自定义提示”单独输入。
6. AI 全文分析、角色音色设计和完整音频渲染由 Node 创建独立 Python Worker 任务。
7. 最近一次完整音频、分轨包和导演清单可以直接预览或下载。
8. 全篇纠音规则、角色补充、旧工程枚举迁移和 GPU 任务互斥。
9. 任务运行时弹出高对比前景进度面板，并锁定当前工程的所有编辑入口；任务结束后自动恢复。
10. 角色表的音色 ID 单元格提供微型播放器，可直接播放、暂停和拖动进度。
11. 枚举下拉菜单使用独立滚动边界，菜单滚动不会带动页面。
12. 运行中任务刷新页面后会自动回到任务所属工程，恢复同一进度与只读锁。活动作业标记位于 `runtime-output/product-jobs/active-job.json`，正常终态自动清理。
13. 角色轨道显示人物小传完整度和声音导演方案。点击“编辑人物与音色”可修改人物身份、小传、音色模式、声音提示、角色节奏和重新生成设置，并查看最终 VoiceDesign 指令。
14. 角色表达节奏与下次生成处理上下排列。验收时应选择完整的“沉稳舒缓，重音清晰，短语间自然停连”长标签，确认选择框、开关和状态说明没有覆盖。
15. 生成角色音色前会显示“正在用 AI 分配导演补充的角色影响范围”。角色编辑窗口的“AI 语义分配”和“本角色有效补充”会显示模型、目标轨道和理由；等待路由或过期的补充不会进入音色指令。
16. 明确女性或男性的角色会显示声音性别硬约束。生成结果经过基频校验后才注册为永久音色；连续三次明显错配会显示错误并保留原音色。
