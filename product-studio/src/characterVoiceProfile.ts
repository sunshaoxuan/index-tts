  ...
  return {
    ...
    standard_reference: input?.standard_reference?.source_voice_id ? {
      ...
      candidates: Array.isArray(input.standard_reference.candidates) ? input.standard_reference.candidates.filter(candidate => candidate?.voice_id).slice(0, 3).map((candidate, index) => ({
        ...
        duration_seconds: Math.max(0, Number(candidate.duration_seconds) || 0),
        audio_quality_passed: Boolean(candidate.audio_quality_passed),
        speaker_similarity: Number(candidate.speaker_similarity) || 0,
        speaker_similarity_threshold: Number(candidate.speaker_similarity_threshold) || 0.72,
        speaker_verified: Boolean(candidate.speaker_verified),
        echo_similarity: Math.max(0, Number(candidate.echo_similarity) || 0),
        echo_threshold: Number(candidate.echo_threshold) || 0.72,
        echo_verified: Boolean(candidate.echo_verified),
        quality_passed: Boolean(candidate.quality_passed),
        score: Number(candidate.score) || 0,
        selected: Boolean(candidate.selected),
        generated_at: String(candidate.generated_at || input.standard_reference?.generated_at || ''),
      })) : [],
      ...(input.standard_reference.adopted_voice_id ? { adopted_voice_id: String(input.standard_reference.adopted_voice_id) } : {}),
      ...(input.standard_reference.adopted_at ? { adopted_at: String(input.standard_reference.adopted_at) } : {}),
      ...(input.standard_reference.restored_at ? { restored_at: String(input.standard_reference.restored_at) } : {}),
    } : undefined,
