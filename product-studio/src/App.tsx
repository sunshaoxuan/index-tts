...
function Studio() {
  ...
  const pitchConstraint = roleAssetDraft ? `角色年龄设定：约 ${roleAssetDraft.age} 岁。建议基频区间：${roleAssetDraft.pitch_min_hz} 至 ${roleAssetDraft.pitch_max_hz} Hz；目标基频中位数约 ${roleAssetDraft.pitch_target_hz} Hz；系统会在保留角色年龄与声音身份门禁的前提下校准并复测基频。${ageVoiceConstraint(roleAssetDraft.age)}` : '';
      ...
      <Modal className="role-editor-modal" width={1120} title={roleDraft ? `${roleDraft[1]} · 角色资产卡片` : '角色资产卡片'} open={roleEditorIndex !== undefined && Boolean(roleDraft)} okText="应用角色设置" cancelText="取消" okButtonProps={{ disabled: jobRunning }} onOk={applyRoleDraft} onCancel={() => { setRoleEditorIndex(undefined); setRoleDraft(undefined); setRoleAssetDraft(undefined); }}>
        {roleDraft && roleAssetDraft && presets && project && <div className="role-editor-grid">
          <section className="role-editor-fields">
            ...
            <div className="editor-section-heading"><span>03 / Voice</span><strong>声音特征与频率目标</strong><Text>性别和年龄会产生建议基频区间。滑块设置候选必须达到的目标基频中位数；系统会校准并复测一至六个通过年龄、性别和目标频率校验的候选，默认生成三个。候选由使用者试听后定稿。年龄约束同时控制共鸣、声带厚度和明亮度。</Text></div>
            ...
            {Boolean(roleAssetDraft.voice_candidates?.length) && <div className="voice-candidate-panel"><Text strong>最近保留的声音候选</Text><Text>可以逐个试听并采用。每个候选都显示原始基频、校准量、目标偏差和容差。儿童候选需要先由试听确认男童或女童身份。</Text>{roleAssetDraft.voice_candidates?.map((candidate, index) => <div className="voice-candidate-row" key={candidate.voice_id}><div><Text>候选 {index + 1} · Seed {candidate.seed}{candidate.median_pitch_hz ? ` · 校准后 ${candidate.median_pitch_hz.toFixed(1)} Hz` : ''} · {candidateVerificationLabel(roleAssetDraft.age, roleAssetDraft.gender, candidate)}</Text>{candidatePitchAuditLabel(candidate) && <Text type="secondary">{candidatePitchAuditLabel(candidate)}</Text>}<VoicePreview voiceId={candidate.voice_id} /></div><Button disabled={jobRunning} type={candidate.voice_id === roleDraft[5] ? 'primary' : 'default'} onClick={() => selectVoiceCandidate(candidate.voice_id)}>{candidate.voice_id === roleDraft[5] ? '当前采用' : roleAssetDraft.age < 13 && roleAssetDraft.gender !== 'unspecified' ? `确认${roleAssetDraft.gender === 'male' ? '男童' : '女童'}并采用` : '采用此候选'}</Button></div>)}</div>}
