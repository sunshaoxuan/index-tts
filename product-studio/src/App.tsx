...
import { applyVoiceCandidateSelection, candidateVerificationLabel } from './voiceCandidateSelection';
      ...
      <Modal className="role-editor-modal" width={1120} title={roleDraft ? `${roleDraft[1]} · 角色资产卡片` : '角色资产卡片'} open={roleEditorIndex !== undefined && Boolean(roleDraft)} okText="应用角色设置" cancelText="取消" okButtonProps={{ disabled: jobRunning }} onOk={applyRoleDraft} onCancel={() => { setRoleEditorIndex(undefined); setRoleDraft(undefined); setRoleAssetDraft(undefined); }}>
        {roleDraft && roleAssetDraft && presets && project && <div className="role-editor-grid">
          <section className="role-editor-fields">
            ...
            {Boolean(roleAssetDraft.voice_candidates?.length) && <div className="voice-candidate-panel"><Text strong>最近保留的声音候选</Text><Text>可以逐个试听并采用。儿童候选需要先由试听确认男童或女童身份。</Text>{roleAssetDraft.voice_candidates?.map((candidate, index) => <div className="voice-candidate-row" key={candidate.voice_id}><div><Text>候选 {index + 1} · Seed {candidate.seed}{candidate.median_pitch_hz ? ` · ${candidate.median_pitch_hz.toFixed(1)} Hz` : ''} · {candidateVerificationLabel(roleAssetDraft.age, roleAssetDraft.gender, candidate)}</Text><VoicePreview voiceId={candidate.voice_id} /></div><Button disabled={jobRunning} type={candidate.voice_id === roleDraft[5] ? 'primary' : 'default'} onClick={() => selectVoiceCandidate(candidate.voice_id)}>{candidate.voice_id === roleDraft[5] ? '当前采用' : roleAssetDraft.age < 13 && roleAssetDraft.gender !== 'unspecified' ? `确认${roleAssetDraft.gender === 'male' ? '男童' : '女童'}并采用` : '采用此候选'}</Button></div>)}</div>}
