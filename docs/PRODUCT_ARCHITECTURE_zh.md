...
OpenAI 兼容服务配置位于 `runtime-output/product-settings.json`。`GET /api/settings/ai-media` 只返回 Endpoint、模型名、文本接口模式、Cockpit Instance ID、传输风险状态和 `hasApiKey`；`PUT` 写入或清除本机密钥。图像路由配置保存 `image_model`、`image_fallback_model` 和 `image_fallback_enabled`。人物小传从角色名字命中的句子向前后各取两句并限制为 30000 字符，可显式选择 `/v1/responses` 或 `/v1/chat/completions`。角色形象和没有画面人物的关键帧按模型族选择传输：GPT Image 与普通兼容模型使用 `/v1/images/generations`，Gemini Image 使用 `/v1/chat/completions` 并通过 `generationConfig.responseModalities` 请求图像输出。含有画面人物的关键帧继续使用 multipart `/v1/images/edits`，以重复 `image[]` 字段发送角色身份参考图。图像响应兼容 `b64_json`、远程 URL、Markdown Data URL、Chat Completions 图像数组和 Gemini inline data，限制图像为 PNG、JPEG 或 WebP 且不超过 20 MB。
...
人物审校把非旁白人物按每批最多两人拆成快速小任务，每批只携带命中当前人物的文章证据、关联文章证据和全局精简身份索引。每批单独执行结构与字段变化校验，未落实的修正只重做当前批次；已经完整解决问题的 `corrected` 由程序验收后直接接受，不再为确认结果发起全人物复核。人物已经使用推断型 age basis、保留非空年龄证据，且 issue 只重复指出原文未明示年龄时，确定性协调器依据审校规则清除重复 issue。当前文章的历史时间点年龄被错误标为 `current_explicit` 时，协调器降级为 `current_inference` 并补充推断说明。具体年龄目标冲突、证据缺失和其他字段问题继续进入当前小批次重做或失败门禁。metrics 分别记录体裁、注册、每个正文块和人物复核小批次，并汇总整项任务的请求数、输入 token、输出 token 与模型耗时。

小说与故事的旁白人口属性使用与人物注册一致的性别证据结构。确定性协调器先从第一人称自述和伴侣关系补全旁白语境推断，并把模型已给出的二元性别和遗漏的 basis 规范化为 `current_inference`。仍为 `unspecified` 的角色组成一次紧凑性别建议请求，只携带稳定 ID、名称、短小传、声音提示和命中原文；合法结果写回角色注册表。请求失败或结果覆盖不完整时保存明确的系统默认声音建议和人工确认标记。`apply_analysis_demographics` 同时处理 `character` 与 `narrator`，旁白可以只更新性别并保留现有年龄；新闻与评论的 `anchor` 不从稿件人物推断人口属性。
...
VoiceDesign 作业读取 `character_assets`，把年龄、建议区间和目标基频写入自然语言指令，并按年龄段补充声带厚度、共鸣位置、亮度及自然质感约束。Worker 根据目标在建议区间中的相对位置追加较低、中间或较高自然音域类别。男性高音目标会追加自然男高音、较高日常说话音域以及口腔与头腔混合共鸣要求。首次尝试使用用户配置的原生采样参数和种子。首次候选偏离目标后，重试指令只保留声音身份、自然音域、声音导演、表达节奏和上次实测偏差，原生采样下限提升到 Top K 100、Top P 0.98、Temperature 1.05。每次尝试使用用户种子加角色累计尝试数形成连续审计序列，跨任务不会重复已经使用的种子。每个模型原始输出先写入 WAV，再由锁定版本的声学依赖重新读取和测量。动态容差为目标值的百分之五，下限三 Hz，上限十 Hz。落盘原始音频同时通过年龄、声音身份和目标频率门禁后才具备候选资格。系统不执行电子变调、相位声码器校准或其他改变音色身份的后处理。前次候选低于或高于目标时，下一次生成指令会要求模型自然提高或降低基频，并明确排除假声、叠音、回声和不自然共鸣。产品 Python 与 VoiceDesign Python 锁定相同的 librosa、NumPy、Numba、SciPy、SoundFile 和 SoXR 版本，运行时校验版本一致性。候选数量由角色配置决定，范围为一至六个。声音签名包含完整指令、生成参数和自然频率门禁版本，因此性别、年龄、目标频率、采样参数、门禁算法或分析依赖变化会建立新缓存签名。人物形象字段不进入声音签名，也不触发音频片断失效。
...
VoiceDesign Worker 把角色配置的一至六解释为合格候选数量，并在一个驻留模型中使用可审计种子自然生成。存在目标基频的角色默认最多尝试本轮缺少数量的十二倍，总数不超过三十六次；只有显式性别时默认最多尝试缺少数量的三倍。收集足量合格候选后立即停止。每次尝试记录种子、落盘原始实测中位基频、目标差值、动态容差、固定为零的修正半音数、固定为 `none` 的修正方法、年龄与声音身份门禁结果和目标频率门禁结果。产品 Worker 立即把每个同时通过年龄、声音身份和目标频率门禁的候选注册到永久音色库并写回角色资产，未通过的尝试留在本次 Worker 审计数据。本轮未补齐时，角色资产保存 `voice_candidate_generation_incomplete` 与累计 `voice_generation_attempts`；下一轮保留已有合格候选，只请求缺少数量，并把累计值作为种子偏移。任务摘要分别统计身份或年龄淘汰数和目标频率淘汰数。用户选择既有候选只改变该角色当前 `voice_id`。
...
标准角色参考样本使用独立的 `standardize` 任务，通过 `POST /api/projects/:id/roles/:roleId/standard-reference` 进入与完整渲染共用的 IndexTTS 模型调度队列。任务元数据保存角色 ID，页面首次提交、服务重启接管和浏览器刷新恢复均可重新关联对应角色卡片。Worker 只读取 `character_assets.<roleId>.reference_audio.voice_id` 指向的原始上传 WAV。前端提供自然、舒缓和自定义节奏，自然与舒缓分别载入 1.05 和 1.18，并用 0.80 至 1.40、步长 0.01 的滑块和数字输入调整实际 `duration_factor`；Node 和 Worker 分别按模型原生 0.50 至 2.00 范围复核有限数值。请求中的实际系数统一进入三版推理、永久候选元数据和工程角色资产，缺少该字段的旧队列请求继续按预设默认值执行。身份优先推理固定使用情绪引导强度 0.25、采样温度 0.60、`top_k=30` 和 `top_p=0.80`，情绪基向量按原始说话人相似度选择。隔离探针使用同一正式音色时，身份优先配置八次全部通过 0.72 音色门禁，原情绪强度 0.45 的八次对照全部未通过。每轮最多尝试三十次，仅累计同时通过基础 WAV 质量、CAMPPlus 音色相似度和延迟回声代理的结果，收齐三版后立即停止并按综合评分排序；预算结束仍不足三版时终止本次更新并保留原有标准候选。音色相似度门槛为 0.72，回声代理上限为 0.72。每次尝试的指标与通过状态写入任务 `result.json` 审计，失败音频随 staging 目录清理；永久 `voice-standard-*` 元数据和角色资产只登记合格候选。前端把三版全门禁候选作为原子交付集合，历史数据只有一版或两版合格时整组隐藏播放器和采用操作，同时显示实际合格数量并要求重新生成。
