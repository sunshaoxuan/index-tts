...
桌面分句表保留 Ant Design 的 560px 初始纵向滚动值作为安全回退。进入分句导演页签后，前端读取视口高度和表格顶部坐标，扣除 132px 的表头、分页及底部安全空间，把结果写入 `--segment-table-body-height`。结果下限为 560px。ResizeObserver 和窗口 resize 事件负责在容器或窗口尺寸变化后重新计算。800px 及以下由后置响应式规则把正文最大高度恢复为无上限，继续使用页面自然滚动。
