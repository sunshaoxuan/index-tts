...
VoiceDesign 作业从本角色的 AI 分配结果、人物小传和声音导演提示推断明确性别。门禁阈值同时读取年龄和性别，儿童使用尚未变声区间，青少年使用变声阶段区间，成年人使用成年性别区间。系统按角色配置使用一至六个连续种子生成候选，避免首个只满足宽泛频率容差的候选被立即接受。通过校验的永久音色元数据保存 `expected_gender`、`median_pitch_hz`、`gender_verified`、`generation_attempts` 和逐候选 `candidate_metrics`，缺少验证信息的显式性别缓存不会复用。
...
VoiceDesign Worker 按角色配置生成一至六个连续种子候选，在一个驻留模型中完成全部候选。每个候选先写入独立 WAV，再记录种子、实测中位基频、目标差值和性别门禁结果。产品 Worker 把全部候选注册到永久音色库，把选中的候选写回角色表，并把候选列表写回角色资产。用户选择既有候选只改变该角色当前 `voice_id`。
