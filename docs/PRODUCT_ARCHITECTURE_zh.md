...
项目生成操作使用右侧固定操作组和收缩图标两种视图。滚动或调整窗口大小时读取完整 `#project` 工作区的边界；从主画面进入项目区时自动展开，从项目区回到主画面时自动收缩。使用者可在完整面板内手动收缩，并可点击右侧图标重新展开；工作区可见性保持不变期间保留手动选择，离开并再次进入项目区时恢复自动展开。三个按钮直接复用 `runJob` 入口、活动任务锁和工程数据前置条件；项目控制栏不再保留重复按钮。顶部任务进度层保持更高层级。
