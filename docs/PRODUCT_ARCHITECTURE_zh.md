...
AI 分句验证完成后，系统只对 AI 生成的合成文本执行句末叹号规范化。中文与日文使用句号 `。`，英文、西班牙文与阿拉伯文使用句点 `.`；句内叹号、原文 `source_text` 和人工编辑的合成文本保持原值。IndexTTS 前端虽会把中文叹号映射为 `!` 并把它作为分段标记，产品仍将感情色彩交给基础情绪、情绪演绎和权重，将片断间硬停顿交给 `pause_after_ms`。该规则在 AI 结果进入分句表前生效，使页面显示、缓存签名和实际送入模型的文本一致。
...
分句数组第 15 至 18 列保存重音文字、出现序号、重音强度和生成方式。渲染器把重音目标追加到 `emo_text`，并将这些字段、生成方式和验收算法版本加入缓存签名。高级生成在最多三十次内持续收集通过基础质量、参考音色和适用重音代理门禁的候选，收齐三版后立即结束；预算耗尽仍不足三版时终止本次更新并保留原片断。代理验收比较目标估计区间与相邻区间的 RMS 能量，保存 `stress_db`、质量指标、排序和算法版本。该指标用于受约束抽卡排序，产品界面始终披露其概率性质。分句草稿索引存在同一序号的多个历史缓存时，最新写入项覆盖旧项进入页面，避免重复显示同一分句。
...
分句高级三版生成的每次 IndexTTS 调用固定传入角色当前稳定参考 WAV，并使用 `use_random=false` 让情绪嵌入原型继续由该参考音色的 CAMPPlus style 选择。三版差异来自语音生成采样。每次尝试在落盘后执行基础音频、参考音色和可选重音门禁；分句参考音色余弦相似度硬门槛为 0.82。系统在最多三十次内持续补生成，收齐三个全门禁候选后立即写入三选一集合，未收齐时终止本次更新并保留原片断。每次尝试向任务状态写入次数与通过数量，工程过程目录保存逐次门禁和异常审计。缓存签名包含参考 WAV 摘要、门槛和校验器版本，因此门槛变化会使旧候选缓存失效。
...
分句导演批量工具使用“字段选择、动态值控件、应用范围”三段式交互，避免把九组输入同时堆入工具栏。纯状态更新函数按稳定分句序号处理全部范围或跨分页选择范围，并返回目标数与实际变化数。角色批量更新同时写入角色 ID 和显示名称；具体情绪演绎预设同时写入建议权重，`auto` 与 `custom` 保留原权重；停顿和情绪权重分别执行 0 至 3000 与 0 至 1 的边界校验。状态函数保留未命中行及未变化行的对象引用，整批无变化时返回原数组，使前端不会产生无效 dirty 状态。合成文字、细化描述和重音目标继续留在逐句编辑区。
