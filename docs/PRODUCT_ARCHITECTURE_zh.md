...
VoiceDesign 作业从本角色的 AI 分配结果、人物小传和声音导演提示推断明确性别。女性音色基频中位数低于 135 Hz、男性音色高于 210 Hz 时视为明显错配，最多使用三个连续种子重试。儿童和 50 岁以上角色完整评估三个连续种子，避免首个只满足宽泛频率容差的候选被立即接受。通过校验的永久音色元数据保存 `expected_gender`、`median_pitch_hz`、`gender_verified`、`generation_attempts` 和逐候选 `candidate_metrics`，缺少验证信息的显式性别缓存不会复用。
...
VoiceDesign 作业读取 `character_assets`，把年龄、建议区间和目标基频写入自然语言指令，并按年龄段补充声带厚度、共鸣位置、亮度及自然质感约束。生成进程继续执行声学性别门禁，并以实测中位基频到目标的绝对距离选择最佳候选。儿童和 50 岁以上角色完整评估三个候选；其他角色可在候选进入频率容差后提前结束。声音签名包含完整指令，因此性别、年龄和目标频率变化会建立新缓存签名。人物形象字段不进入声音签名，也不触发音频片断失效。
