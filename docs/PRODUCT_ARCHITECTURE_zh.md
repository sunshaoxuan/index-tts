...
分句高级三版生成的每次 IndexTTS 调用固定传入角色当前稳定参考 WAV，并使用 `use_random=false` 让情绪嵌入原型继续由该参考音色的 CAMPPlus style 选择。三版差异来自语音生成采样。每次尝试在落盘后执行基础音频、参考音色和可选重音门禁；分句参考音色余弦相似度硬门槛为 0.82。最多九次内收齐三个全门禁候选后才写入三选一集合，未收齐时终止本次更新并保留原片断。缓存签名包含参考 WAV 摘要、门槛和校验器版本，因此门槛变化会使旧候选缓存失效。
