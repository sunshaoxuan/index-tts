...
VoiceDesign 作业读取 `character_assets`，把年龄、建议区间和目标基频写入自然语言指令，并按年龄段补充声带厚度、共鸣位置、亮度及自然质感约束。生成进程先对原始声音执行年龄感知的声学门禁，随后使用高质量相位声码器把中位基频校准到用户目标并重新测量。动态容差为目标值的百分之五，下限三 Hz，上限十 Hz；复测结果进入容差后才具备候选资格。候选数量由角色配置决定，范围为一至六个。声音签名包含完整指令、生成参数和基频校准版本，因此性别、年龄、目标频率、采样参数或校准算法变化会建立新缓存签名。人物形象字段不进入声音签名，也不触发音频片断失效。
