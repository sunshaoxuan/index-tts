...
VoiceDesign 作业读取 `character_assets`，把年龄、建议区间和目标基频写入自然语言指令，并按年龄段补充声带厚度、共鸣位置、亮度及自然质感约束。生成进程先对原始声音执行年龄感知的声学门禁，随后使用高质量相位声码器把中位基频校准到用户目标。候选写入 WAV 后由同一锁定版本的声学依赖重新读取、测量，落盘值偏出容差时继续校准和复测。动态容差为目标值的百分之五，下限三 Hz，上限十 Hz；最终落盘复测结果进入容差后才具备候选资格。产品 Python 与 VoiceDesign Python 锁定相同的 librosa、NumPy、Numba、SciPy、SoundFile 和 SoXR 版本，运行时校验版本一致性。候选数量由角色配置决定，范围为一至六个。声音签名包含完整指令、生成参数和基频校准版本，因此性别、年龄、目标频率、采样参数、校准算法或分析依赖变化会建立新缓存签名。人物形象字段不进入声音签名，也不触发音频片断失效。
...
VoiceDesign Worker 把角色配置的一至六解释为合格候选数量，并在一个驻留模型中使用连续种子生成。存在显式性别或目标基频的角色最多尝试请求数量的三倍，收集足量合格候选后立即停止。每次尝试记录种子、原始实测中位基频、校准后中位基频、目标差值、动态容差、校准半音数、校准方法、年龄与声音身份门禁结果和目标频率门禁结果。产品 Worker 只把同时通过年龄、声音身份和目标频率门禁的候选注册到永久音色库并写回角色资产，未通过的尝试留在本次 Worker 审计数据。用户选择既有候选只改变该角色当前 `voice_id`。
