...
角色轨道使用稳定角色 ID 保存当前焦点。工程切换、角色删除和任务结果重载时，`normalizeActiveRoleId` 保留仍存在的当前角色，失效时回到第一条。表格行的点击、子控件焦点、试听、编辑入口、Enter 和空格共同更新该 ID；`aria-selected`、当前重点摘要、整行暖棕背景和左侧暖奶油标记使用同一状态。

角色表横向滚动时，固定轨道 ID 列使用局部暖黑阅读底，悬停与当前重点状态切换为暖棕底。该局部表格表面阻止后方角色列文字透入固定列，工作区外层继续保持透明。
