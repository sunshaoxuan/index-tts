...
| 新请求不干预 Ollama 生命周期 | Python 请求体回归测试断言无 `keep_alive` 和 `num_ctx` | 待最终测试记录 |
| 提交后异步轮询 | Node 集成测试断言 HTTP 202、持久状态、依赖、模型优先和结果查询 | 待最终测试记录 |
| 生产 runner 稳定 | 新镜像真实全文分析与 Ollama 日志 | 待部署验收 |
